...
const getDatabaseUrl = (): string => {
    ...
    if (schemaEnv.includes("sla-db") || schemaEnv.includes("sla_db")) {
        try {
            return env("SLA_DATABASE_URL");
        } catch {
            throw new Error("SLA_DATABASE_URL must be set in .env file");
        }
    ...
    // Priority 4: Fallback to schema-specific env vars (default to sla-service)
    try {
        return env("SLA_DATABASE_URL");
    } catch {
        try {
            ...
            throw new Error(
                "PRISMA_DATABASE_URL, DATABASE_URL, or schema-specific environment variables (SLA_DATABASE_URL, DATA_LOGGERS_DATABASE_URL) must be set in .env file."
            );
        }
    }
};
...
const getSchemaPath = (): string => {
    ...
    // Priority 2: Default to sla-db schema
    return "prisma/sla-db/schema.prisma";
};
